...
installChaincode () {
	...
	VERSION=${3:-1.0}
        set -x
    echo "peer chaincode install -n mycc -v ${VERSION} -l ${LANGUAGE} -p ${CC_SRC_PATH} >&log.txt"
...
upgradeChaincode () {
    ...
    setGlobals $PEER $ORG
    endorse=''
    for ((i=1; i<=$ORGID; i++)); do
        if [ $i -eq $ORGID ]; then
            endorse="$endorse'Org${i}MSP.peer'"
        else
            endorse="$endorse'Org${i}MSP.peer',"
        fi
    done
    set -x
    peer chaincode upgrade -o orderer.example.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n mycc -v ${ORGID} -c '{"Args":["init","a","90","b","210"]}' -P "OR ($endorse)"
    res=$?
	set +x
    cat log.txt
    verifyResult $res "Chaincode upgrade on org${ORGID} peer${PEER} has Failed"
    echo "===================== Chaincode is upgraded on org${ORGID} peer${PEER} ===================== "
    echo
}
